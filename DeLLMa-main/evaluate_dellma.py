...
def parse_rank_prompt_response(
    ...
    if preference_config.pref_enum_mode in ["rank", "rank-minibatch"]:
        if "minibatch" in preference_config.pref_enum_mode:
            ...
            config_path = ""
        combo_str = "-".join(choices)
        base = os.path.join(
            results_path, domain_path, "dellma", preference_config.pref_enum_mode
        )
        # Avoid ''/{config_path}/'' → '//' when config_path is empty (rank vs rank-minibatch).
        if config_path:
            result_path = os.path.join(base, config_path, combo_str)
        else:
            result_path = os.path.join(base, combo_str)
    else:
