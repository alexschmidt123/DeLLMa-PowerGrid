...
# Project root = DeLLMa-main (parent of agent/)
...
- explanation: a string that describes, in detail, the reasoning behind your decision. """
            ...
            elif self.product == "microgrid_regime":
                if getattr(self, "reveal_stability_stats", False):
...
"""
    ...
    {{
        ...
        ...
            ...
            if self.product == "microgrid_regime":
                if getattr(self, "reveal_stability_stats", False):
                    reward_prompt = f"""{prefix}For each action, estimate the expected **mean stab** from that regime's sample if you commit to that microgrid regime (stab<0 is stable, stab>0 is unstable; higher mean indicates higher instability risk).
        ...
        Each key should map to a JSON object with keys 'reward' (float; use **mean stab** so larger reward = more unstable), 'unit' (e.g. 'mean_stab'), and 'explanation' (your reasoning)."""
                else:
